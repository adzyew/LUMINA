...
@section('content')
    <header class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-playfair font-bold text-gray-900 dark:text-white">All Products</h1>
            <p class="text-gray-600 dark:text-gray-400 text-sm mt-1">Manage your product catalog.</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border border-gray-200 dark:border-white/5">
                ...
                <tr class="hover:bg-amber-300/12 transition duration-300">
                    ...
                    <td class="p-4">
                        <div class="flex justify-center gap-3">
                            <a href="{{ route('admin.products.show', $product->id) }}" class="text-gray-700 dark:text-white hover:text-gray-500" title="view">
        ...
        <form action="{{ route('admin.products.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Permanently delete this product? This cannot be undone.');">
            @csrf
            @method('DELETE')
            <button type="submit" class="text-red-500 hover:text-red-400 transition-colors" title="Delete permanently">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8">
