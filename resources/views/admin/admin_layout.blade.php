...
<body class="bg-[#F8F8F8] text-gray-900 dark:bg-gray-900 dark:text-gray-100 font-sans antialiased flex h-screen overflow-hidden transition-colors">
    ...
    <aside class="w-64 bg-[#F8F8F8]/95 backdrop-blur-md order-r border-white/10 hidden md:flex flex-col shrink-0 flex-shrink-0">
        @php
        ...
        <div class="p-6 flex flex-col gap-1 border-b border-white/10 min-h-[5rem] justify-center">
            ...
            @if($staffDepartment)
                <span class="text-xs text-black font-medium">{{ $staffDepartment }} Department</span>
            @endif
        </div>
        ...
        <nav class="grow p-2 space-y-2">
            ...
            @endphp
            <a href="{{ $dashboardUrl }}" class="flex items-center gap-3 px-4 py-3 {{ $dashboardActive ? 'bg-amber-300 text-black font-bold' : 'text-black hover:text-amber-300 font-bold' }} rounded-lg transition-colors">
            ...
            @can('inventory.view')
            <a href="{{ route('admin.products.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.products.*') ? 'bg-amber-300 text-black font-bold' : 'text-black hover:text-amber-300 font-bold' }} rounded-lg transition-colors">
            ...
            @can('sales.view')
            <a href="{{ route('admin.orders.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.orders.*') ? 'bg-amber-300 text-black font-bold' : 'text-black hover:text-amber-300 font-bold' }} rounded-lg transition-colors">
            ...
            <a href="{{ route('admin.analytics.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.analytics.*') ? 'bg-amber-300 text-black font-bold' : 'text-black hover:text-amber-300 font-bold' }} rounded-lg transition-colors">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                Analytics
            </a>
            <a href="{{ route('admin.sales.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.sales.*') ? 'bg-amber-300 text-black font-bold' : 'text-black hover:text-amber-300 font-bold' }} rounded-lg transition-colors">
            ...
            <a href="{{ route('admin.deliveries.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.deliveries.*') ? 'bg-amber-300 text-black font-bold' : 'text-black hover:text-amber-300 font-bold' }} rounded-lg transition-colors">
            ...
            @role('admin')
                <a href="{{ route('admin.users.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.users.*') ? 'bg-amber-300 text-black font-bold' : 'text-black hover:text-amber-300 font-bold' }} rounded-lg transition-colors">
                ...
                <a href="{{ route('admin.roles.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.roles.*') ? 'bg-amber-300 text-black font-bold' : 'text-black hover:text-amber-300 font-bold' }} rounded-lg transition-colors">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
