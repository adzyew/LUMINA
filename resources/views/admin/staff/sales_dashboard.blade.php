    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <div class="bg-white dark:bg-gray-900/60 rounded-3xl p-6 border border-gray-200 dark:border-white/5 shadow-lg relative overflow-hidden">
            ...
            <div class="flex items-center gap-4 mb-4">
                ...
                <h3 class="text-gray-500 dark:text-gray-400 font-medium">Revenue (This Month)</h3>
            </div>
        ...
        <div class="bg-white dark:bg-gray-900/60 rounded-3xl p-6 border border-gray-200 dark:border-white/5 shadow-lg relative overflow-hidden">
            <div class="flex items-center gap-4 mb-4">
                ...
                <h3 class="text-gray-500 dark:text-gray-400 font-medium">Lifetime Revenue</h3>
            </div>
            <p class="text-3xl font-bold text-gray-900 dark:text-white">₱{{ number_format($totalRevenue, 2) }}</p>
        </div>
    </div>

    <div class="bg-white dark:bg-gray-900/80 rounded-4xl p-6 sm:p-8 border border-gray-200 dark:border-white/5 shadow-xl mb-10">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">Order Status Overview</h2>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <div class="p-4 rounded-xl border border-amber-500/20 bg-amber-500/10">
                ...
                <p class="text-2xl font-bold text-amber-700 dark:text-amber-300">{{ $salesStatuses['pending'] ?? 0 }}</p>
            </div>
            <div class="p-4 rounded-xl border border-blue-500/20 bg-blue-500/10">
                <p class="text-xs uppercase tracking-wide text-blue-600 dark:text-blue-400">Confirmed</p>
                <p class="text-2xl font-bold text-blue-700 dark:text-blue-300">{{ $salesStatuses['confirmed'] ?? 0 }}</p>
            </div>
            <div class="p-4 rounded-xl border border-indigo-500/20 bg-indigo-500/10">
                ...
                <p class="text-2xl font-bold text-indigo-700 dark:text-indigo-300">{{ $salesStatuses['processing'] ?? 0 }}</p>
            </div>
            <div class="p-4 rounded-xl border border-purple-500/20 bg-purple-500/10">
                <p class="text-xs uppercase tracking-wide text-purple-600 dark:text-purple-400">Shipped</p>
                <p class="text-2xl font-bold text-purple-700 dark:text-purple-300">{{ $salesStatuses['shipped'] ?? 0 }}</p>
            </div>
            <div class="p-4 rounded-xl border border-green-500/20 bg-green-500/10">
                ...
                <p class="text-2xl font-bold text-green-700 dark:text-green-300">{{ $salesStatuses['delivered'] ?? 0 }}</p>
            </div>
            <div class="p-4 rounded-xl border border-red-500/20 bg-red-500/10">
                ...
                <p class="text-2xl font-bold text-red-700 dark:text-red-300">{{ $salesStatuses['cancelled'] ?? 0 }}</p>
            </div>
        </div>
    </div>

    <div class="bg-white dark:bg-gray-900/80 rounded-4xl p-6 sm:p-8 border border-gray-200 dark:border-white/5 shadow-xl">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">Recent Orders to Process</h2>
                ...
                <tbody class="text-gray-600 dark:text-gray-300">
                    @forelse($recentOrders as $order)
                        <tr class="border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                            <td class="py-4 px-4 font-bold text-gray-900 dark:text-white">#{{ $order->id }}</td>
                            <td class="py-4 px-4">
                            ...
                            <td class="py-4 px-4 text-center">
                                <a href="{{ route('admin.orders.show', $order->id) }}" class="inline-flex items-center justify-center px-4 py-2 bg-gray-100 dark:bg-white/10 hover:bg-gray-200 dark:hover:bg-white/20 text-gray-700 dark:text-white text-sm font-semibold rounded-full transition-colors">
                                    Review
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="py-12 text-center text-gray-500">
                                <svg class="w-12 h-12 mx-auto mb-3 text-gray-400 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
