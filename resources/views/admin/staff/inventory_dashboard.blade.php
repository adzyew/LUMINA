...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        <h3 class="text-gray-500 text-sm font-medium mb-1">Low Stock (1–5 units)</h3>
        <p class="text-3xl font-bold {{ $lowStock > 0 ? 'text-amber-600' : 'text-gray-900' }}">{{ $lowStock }}</p>
        @if($lowStock > 0)
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <p class="text-3xl font-bold {{ $outOfStock > 0 ? 'text-red-600' : 'text-gray-900' }}">{{ $outOfStock }}</p>
        @if($outOfStock > 0)
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        <h3 class="text-gray-500 text-sm font-medium mb-1">In Stock</h3>
        <p class="text-3xl font-bold text-green-600">{{ $totalProducts - $outOfStock }}</p>
    </div>
</div>
        ...
        <table class="w-full text-left">
            ...
            <tbody class="text-sm">
                ...
                <tr class="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td class="py-4 text-gray-600">{{ $product->category ?? '—' }}</td>
                    <td class="py-4">
                    ...
                    <td class="py-4 text-right text-amber-600 font-bold">₱{{ number_format($product->price ?? 0, 2) }}</td>
                    <td class="py-4 text-right">
                        <a href="{{ route('admin.products.edit', $product) }}" class="text-amber-500 hover:text-amber-400 text-sm">Edit</a>
                    </td>
                </tr>
                @empty
                <tr><td colspan="5" class="py-8 text-center text-gray-500">No products yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
