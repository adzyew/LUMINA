<div class="mb-10 bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/5 rounded-2xl px-6 py-3 shadow-sm dark:shadow-none">
    <div class="flex items-center justify-between gap-4">
        <header class="flex items-center gap-3 min-w-0">
            <h1 class="text-3xl font-playfair font-bold text-gray-900 dark:text-white">{{ $title ?? 'Overview' }}</h1>
            @if(!empty($subtitle))
                <p class="text-gray-600 dark:text-gray-400 text-sm mt-3 truncate">{{ $subtitle }}</p>
            @endif
        </header>
