    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-white border border-gray-200 rounded-2xl p-6">
            ...
            <div class="space-y-3 text-sm">
                <div class="flex justify-between gap-4"><span class="text-gray-500">Name</span><span class="text-gray-900 font-semibold text-right">{{ $user->name }}</span></div>
                <div class="flex justify-between gap-4"><span class="text-gray-500">Email</span><span class="text-gray-900 font-semibold text-right">{{ $user->email }}</span></div>
                <div class="flex justify-between gap-4"><span class="text-gray-500">Phone</span><span class="text-gray-900 font-semibold text-right">{{ $user->phone ?: 'Not set' }}</span></div>
                <div class="flex justify-between gap-4"><span class="text-gray-500">Verified</span><span class="text-right">@if($user->is_verified ?? $user->email_verified_at)<span class="px-2 py-1 rounded-full text-xs font-medium bg-green-500/20 text-green-400">Verified</span>@else<span class="px-2 py-1 rounded-full text-xs font-medium bg-amber-500/20 text-amber-400">Pending</span>@endif</span></div>
                <div class="flex justify-between gap-4"><span class="text-gray-500">Status</span><span class="text-right">@if($user->archived_at)<span class="px-2 py-1 rounded-full text-xs font-medium bg-red-500/20 text-red-400">Archived</span>@else<span class="px-2 py-1 rounded-full text-xs font-medium bg-blue-500/20 text-blue-300">Active</span>@endif</span></div>
                <div class="flex justify-between gap-4"><span class="text-gray-500">Created</span><span class="text-gray-900 font-semibold text-right">{{ optional($user->created_at)->format('M d, Y h:i A') ?: '—' }}</span></div>
                <div class="flex justify-between gap-4"><span class="text-gray-500">Last Updated</span><span class="text-gray-900 font-semibold text-right">{{ optional($user->updated_at)->format('M d, Y h:i A') ?: '—' }}</span></div>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-6">
            ...
            <div>
                <p class="text-xs uppercase tracking-wide text-gray-500 mb-2">Direct Permissions</p>
                <div class="flex flex-wrap gap-2">
        ...
        <div class="lg:col-span-2 bg-white border border-gray-200 rounded-2xl p-6">
            <h2 class="text-lg font-bold text-gray-900 mb-4">Address & Notifications</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div class="rounded-xl bg-gray-50 border border-gray-200 p-4">
                    <p class="text-xs uppercase tracking-wide text-gray-500 mb-2">Shipping Address</p>
                    <p class="text-gray-900 font-semibold">{{ $user->shipping_address ?: 'Not set' }}</p>
                </div>
                <div class="rounded-xl bg-gray-50 border border-gray-200 p-4">
                    <p class="text-xs uppercase tracking-wide text-gray-500 mb-2">Notifications</p>
                    <ul class="space-y-1 text-gray-900 font-semibold">
