        ...
        <div class="flex justify-center items-center gap-2">
            ...
            <button type="button" title="View"
               onclick="showUserViewModal({
                   name: @js($user->name),
                   email: @js($user->email),
                   phone: @js($user->phone ?: 'Not set'),
                   verified: {{ $user->hasVerifiedEmail() ? 'true' : 'false' }},
                   archived: {{ $user->archived_at ? 'true' : 'false' }},
                   created_at: @js(optional($user->created_at)->format('M d, Y h:i A') ?: '—'),
                   updated_at: @js(optional($user->updated_at)->format('M d, Y h:i A') ?: '—'),
                   shipping_address: @js($user->shipping_address ?: 'Not set'),
                   notify_order_updates: {{ $user->notify_order_updates ? 'true' : 'false' }},
                   notify_promotions: {{ $user->notify_promotions ? 'true' : 'false' }},
                   notify_loyalty: {{ $user->notify_loyalty ? 'true' : 'false' }},
                   roles: @js($user->roles->pluck('name')->values())
               })"
