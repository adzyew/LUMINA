...
<div id="deleteRoleModal" class="fixed inset-0 z-50 hidden overflow-y-auto" role="dialog" aria-modal="true">
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="relative z-10 bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xl p-8 max-w-md w-full">
            ...
            <p class="text-gray-600 dark:text-gray-300 mb-6">Are you sure you want to permanently delete the <span id="deleteRoleName" class="font-bold text-white"></span> role?</p>
            <div class="flex justify-end gap-3">
                <button type="button" onclick="hideDeleteModal()" class="px-5 py-2.5 bg-gray-100 dark:bg-white/5 border border-gray-300 dark:border-white/10 hover:bg-gray-200 dark:hover:bg-white/10 text-gray-700 dark:text-white rounded-xl transition-colors font-medium">
                    Cancel
                </button>
...
@if(session('success'))
    <div class="mb-6 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 p-4 rounded-lg border border-green-200 dark:border-green-700/30">
        {{ session('success') }}
    </div>
@endif
@if(session('error'))
    <div class="mb-6 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 p-4 rounded-lg border border-red-200 dark:border-red-700/30">
        {{ session('error') }}
    </div>
@endif
...
<div class="flex gap-2 mb-6">
    <a href="{{ route('admin.roles.index', ['tab' => 'active']) }}"
       class="px-5 py-2 rounded-xl font-semibold text-sm transition-colors
              {{ $tab !== 'archived' ? 'bg-amber-400 text-black shadow' : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 border border-white/10' }}">
        Active
        <span class="ml-1.5 px-2 py-0.5 rounded-full text-xs {{ $tab !== 'archived' ? 'bg-black/20' : 'bg-white/10' }}">{{ $activeRoles->count() }}</span>
    </a>
    <a href="{{ route('admin.roles.index', ['tab' => 'archived']) }}"
       class="px-5 py-2 rounded-xl font-semibold text-sm transition-colors
              {{ $tab === 'archived' ? 'bg-amber-400 text-black shadow' : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 border border-white/10' }}">
        Archived
        <span class="ml-1.5 px-2 py-0.5 rounded-full text-xs {{ $tab === 'archived' ? 'bg-black/20' : 'bg-white/10' }}">{{ $archivedRoles->count() }}</span>
    </a>
</div>
                ...
                <tr class="hover:bg-amber-300/5 transition duration-300">
                    <td class="p-4">
                        ...
                        @if(strtolower($role->name) === 'admin')
                            <span class="ml-2 px-2 py-0.5 bg-amber-500/20 text-amber-400 text-xs rounded-full font-semibold">System</span>
                        @endif
                    </td>
                    <td class="p-4">
                    ...
                    <td class="p-4">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <a href="{{ route('admin.roles.show', $role->id) }}"
                               title="View Role"
                               class="w-9 h-9 flex items-center justify-center rounded-xl bg-amber-400/10 hover:bg-amber-400 text-amber-400 hover:text-black transition-all duration-200">
                            ...
                            @if($tab !== 'archived')
                                ...
                                <a href="{{ route('admin.roles.edit', $role->id) }}"
                                   ...
                                   class="w-9 h-9 flex items-center justify-center rounded-xl bg-blue-500/10 hover:bg-blue-500 text-blue-400 hover:text-white transition-all duration-200">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @if(strtolower($role->name) !== 'admin')
                                    <form id="archive-role-form-{{ $role->id }}" action="{{ route('admin.roles.archive', $role->id) }}" method="POST">
                                        ...
                                        <button type="submit"
                                                ...
                                                class="w-9 h-9 flex items-center justify-center rounded-xl bg-gray-500/10 hover:bg-gray-500 text-gray-400 hover:text-white transition-all duration-200">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @if(strtolower($role->name) !== 'admin')
                                    <form id="delete-role-form-{{ $role->id }}" action="{{ route('admin.roles.destroy', $role->id) }}" method="POST">
                                        ...
                                        <button type="button"
                                                ...
                                                class="w-9 h-9 flex items-center justify-center rounded-xl bg-red-500/10 hover:bg-red-500 text-red-400 hover:text-white transition-all duration-200">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
