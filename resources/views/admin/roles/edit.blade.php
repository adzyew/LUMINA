    ...
    <div class="bg-gray-900 border border-white/5 rounded-2xl p-6 sm:p-8">
        ...
        @method('PUT')
            ...
            @foreach($permissions as $permission)
                    <label class="flex items-center space-x-3 bg-gray-800 border border-white/5 p-4 rounded-lg cursor-pointer hover:bg-gray-800/80 transition-colors">
                    <input type="checkbox"
                           name="permissions[]"
            ...
            Save Changes
                ...
                <a href="{{ route('admin.roles.index') }}" class="px-6 py-2.5 bg-white/5 text-gray-300 font-semibold rounded-lg hover:bg-white/10 border border-white/10 transition-colors">
                    Cancel
                </a>
            </div>
    </form>
