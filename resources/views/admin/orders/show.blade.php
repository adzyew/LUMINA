...
<div class="max-w-4xl w-full">
    <header class="mb-8 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-playfair font-bold text-black">Order #{{ $order->id }}</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div class="bg-gray-900 border border-white/5 rounded-2xl p-6">
            ...
            <dl class="space-y-3 text-sm">
                ...
                <div class="flex justify-between"><dt class="text-gray-400">Email</dt><dd class="text-white">{{ $order->user->email ?? '—' }}</dd></div>
                ...
                <div class="flex justify-between"><dt class="text-gray-400">Tracking</dt><dd class="text-white">{{ $order->tracking_number ?? '—' }}</dd></div>
                @if($order->contact_phone)
                <div class="flex justify-between"><dt class="text-gray-400">Contact</dt><dd class="text-white">{{ $order->contact_phone }}</dd></div>
                @endif
                @if($order->formatted_shipping_address)
                <div><dt class="text-gray-400 mb-1">Shipping Address</dt><dd class="text-white whitespace-pre-line">{{ $order->formatted_shipping_address }}{{ $order->notes ? "\n\nNotes: " . $order->notes : '' }}</dd></div>
                @elseif($order->shipping_address)
                <div><dt class="text-gray-400 mb-1">Shipping Address</dt><dd class="text-white">{{ $order->shipping_address }}{{ $order->notes ? ' — Notes: ' . $order->notes : '' }}</dd></div>
                @endif
            </dl>

            <form method="POST" action="{{ route('admin.orders.update', $order) }}" class="mt-6 pt-6 border-t border-white/10 space-y-4">
                @csrf
                @method('PUT')
                <div>
        ...
        <div class="bg-gray-900 border border-white/5 rounded-2xl p-6">
            ...
            <div class="space-y-4">
                ...
                <div class="flex justify-between items-center py-2 border-b border-white/5">
                    <div>
                        <p class="text-white font-medium">{{ $item->product->name ?? 'Product #'.$item->product_id }}</p>
                        <p class="text-gray-400 text-sm">{{ $item->quantity }} × ₱{{ number_format($item->unit_price, 2) }}</p>
                    </div>
