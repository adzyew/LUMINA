...
@section('content')
<header class="mb-8">
    <h1 class="text-3xl font-playfair font-bold text-black">Orders</h1>
...
<div class="flex flex-wrap gap-2 mb-6">
    <a href="{{ route('admin.orders.index') }}" class="px-4 py-2 rounded-lg text-sm font-semibold transition-colors {{ !request('status') ? 'bg-amber-300 text-black' : 'bg-white/5 text-gray-600 hover:bg-white/10 border border-gray-300' }}">All</a>
    @foreach(['pending','confirmed','processing','shipped','delivered','cancelled'] as $s)
    <a href="{{ route('admin.orders.index', ['status' => $s]) }}" class="px-4 py-2 rounded-lg text-sm font-semibold transition-colors {{ request('status') === $s ? 'bg-amber-300 text-black' : 'bg-white/5 text-gray-600 hover:bg-white/10 border border-gray-300' }}">{{ ucfirst($s) }}</a>
    @endforeach
</div>
    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-white/10">
            ...
            <tr class="hover:bg-amber-300/10 transition duration-300">
                ...
                <td class="p-4 text-gray-400">{{ $order->created_at->format('M d, Y') }}</td>
