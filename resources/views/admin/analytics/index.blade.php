...
<header class="flex justify-between items-center mb-8">
    <div>
        <h1 class="text-3xl font-playfair font-bold text-black">Analytics & CRM</h1>
    ...
    <form action="{{ route('admin.analytics.export') }}" method="GET" class="flex gap-2 items-center">
        <input type="date" name="from" class="bg-gray-800 border border-white/10 rounded-lg px-3 py-2 text-sm text-white" placeholder="From">
        <input type="date" name="to" class="bg-gray-800 border border-white/10 rounded-lg px-3 py-2 text-sm text-white" placeholder="To">
        <select name="status" class="bg-gray-800 border border-white/10 rounded-lg px-3 py-2 text-sm text-white">
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <div class="bg-gray-900 border border-white/5 rounded-2xl p-6">
        <h3 class="text-gray-400 text-sm font-medium mb-1">This Month</h3>
        <p class="text-3xl font-bold text-white">₱{{ number_format($thisMonthRevenue, 2) }}</p>
    </div>
    <div class="bg-gray-900 border border-white/5 rounded-2xl p-6">
        <h3 class="text-gray-400 text-sm font-medium mb-1">Last Month</h3>
        <p class="text-3xl font-bold text-white">₱{{ number_format($lastMonthRevenue, 2) }}</p>
    </div>
    <div class="bg-gray-900 border border-white/5 rounded-2xl p-6">
        <h3 class="text-gray-400 text-sm font-medium mb-1">Revenue Change</h3>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
    <div class="bg-gray-900 border border-white/5 rounded-2xl p-6">
        <h3 class="text-lg font-bold text-white mb-4">Revenue (Last 7 Days)</h3>
        <canvas id="revenueChart" height="200"></canvas>
    </div>
    <div class="bg-gray-900 border border-white/5 rounded-2xl p-6">
        <h3 class="text-lg font-bold text-white mb-4">Orders by Status</h3>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    <div class="bg-gray-900 border border-white/5 rounded-2xl p-6">
        <h3 class="text-lg font-bold text-white mb-4">Top Products</h3>
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr class="border-b border-white/5">
                    <td class="py-3 text-white">{{ $item->product->name ?? 'Product #'.$item->product_id }}</td>
    ...
    <div class="bg-gray-900 border border-white/5 rounded-2xl p-6">
        <h3 class="text-lg font-bold text-white mb-4">Top Customers</h3>
        <table class="w-full text-sm">
            <thead>
                <tr class="text-gray-500 border-b border-white/10">
            ...
            <tbody>
                ...
                <tr class="border-b border-white/5">
                    <td class="py-3 text-white">{{ $customer->name }}</td>
