...
<body class="bg-gray-50 text-gray-900 dark:bg-black dark:text-white font-sans antialiased min-h-screen flex flex-col transition-colors">
    ...
    <main class="grow container mx-auto px-4 sm:px-6 py-12">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            @endphp
            <div>
                <!-- Constrain image to a reasonable max width/height so it fits on smaller screens -->
                <div class="relative  rounded-2xl overflow-hidden border border-white/10 flex items-center justify-center">
                    <div class="w-full max-w-3xl p-4">
                        @if($mainImg)
                            <img id="mainProductImage" src="{{ $mainImg }}" alt="{{ $product->name }}" class="w-full h-auto max-h-[70vh] object-contain mx-auto">
                        @else
                            <div class="w-full h-64 flex items-center justify-center text-gray-600">
                ...
                @if($gallery->count() > 1)
                    <div class="mt-4 grid grid-cols-5 gap-3 overflow-x-auto">
                        @foreach($gallery as $img)
                            <button type="button" onclick="setMainProductImage('{{ $img }}')" class="bg-white/5 rounded-xl border border-white/10 hover:border-amber-300/40 overflow-hidden">
