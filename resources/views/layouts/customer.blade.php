    ...
    <aside id="customerSidebar" class="fixed inset-y-0 left-0 z-40 w-72 -translate-x-full lg:translate-x-0 lg:static bg-white border-r border-amber-100 flex h-screen flex-col transition-transform duration-200 ease-out shadow-sm">
        ...
        <div class="px-6 py-5 border-b border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-gray-400 text-xs mt-0.5 truncate max-w-40">{{ Auth::user()->email }}</p>
                </div>
            </div>
        </div>
        @endauth
        <nav class="px-3 py-4 space-y-4 grow overflow-y-auto">
            <div>
                ...
                <div class="space-y-1">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg {{ request()->routeIs('dashboard') ? 'bg-amber-100/50 text-gray-900 font-semibold border-l-amber-300 border-l-6' : 'text-gray-600  hover:bg-amber-50/70' }} transition-colors">
                    ...
                    <a href="{{ route('orders.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg {{ request()->routeIs('orders.*') ? 'bg-amber-100/50 text-gray-900 font-semibold border-l-amber-300 border-l-6' : 'text-gray-600 hover:bg-amber-50/70' }} transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                    ...
                    <a href="{{ route('wishlist.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg {{ request()->routeIs('wishlist.*') ? 'bg-amber-100/50 text-gray-900 font-semibold border-l-amber-300 border-l-6' : 'text-gray-600  hover:bg-amber-50/70' }} transition-colors">
            ...
            <div>
                ...
                <div class="space-y-1">
                    <a href="{{ $settingsLink }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg {{ $isProfileRoute ? 'text-gray-900 bg-amber-100/50 font-semibold border-l-amber-300 border-l-6' : 'text-gray-600  hover:bg-amber-50/70' }} transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
            ...
            <div>
                ...
                <div class="space-y-1">
                    <a href="{{ route('support.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg {{ request()->routeIs('support.*') ? 'bg-amber-100/50  text-gray-900 font-semibold border-l-amber-300 border-l-6' : 'text-gray-600 hover:bg-amber-50/70' }} transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        ...
        <div class="mt-auto p-4 border-t border-gray-100">
            <form method="POST" action="{{ route('logout') }}" data-logout-form>
                @csrf
                <button type="button" onclick="showLogoutModal()" class="w-full px-4 py-2.5 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition-colors text-md font-semibold">
                    Logout
                </button>
            </form>
        </div>
